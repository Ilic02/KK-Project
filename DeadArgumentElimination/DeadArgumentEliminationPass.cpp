...
struct DeadArgumentEliminationPass
    ...
    PreservedAnalyses run(Module &Mod, ModuleAnalysisManager &) {
        ...
        for (auto Func : deadArgFuncs) {
            ...
            // originalParams - deadParams Keep the index of each surviving
            // parameter for future use
            std::map<int, Type *> survivingParams;
            for (size_t i = 0; i < Func->getFunctionType()->params().size();
                 i++) {
            ...
            // Create a new (or reuse an already created) function type(ret
            // type, params, variadic flag) from the FunctionType Factory
            auto values = std::views::values(survivingParams);
            std::vector<Type *> paramTypes(values.begin(), values.end());
            auto newFuncType = FunctionType::get(Func->getReturnType(),
                                                 paramTypes, Func->isVarArg());
            ...
            // Body is copied over, but the body still references the old
            // arguments. Replace all uses of the old surviving arguments with
            // the new ones
            int newArgIndex = 0;
            for (auto key : std::views::keys(survivingParams)) {
                Func->getArg(key)->replaceAllUsesWith(
                    newFunc->getArg(newArgIndex++));
            }

            // New functions are created, now wire them up.
            // First, replace all calls of the old function with the new
            // function
            for (auto Use : Func->users()) {
                if (auto CB = dyn_cast<CallBase>(Use)) {
                    ...
                    auto newCall =
                        CallInst::Create(newFuncType, newFunc, survivingArgs,
                                         "", CB->getIterator());
                    CB->replaceAllUsesWith(newCall);
                    newCall->takeName(CB);
                    callsToErase.push_back(
                        CB); // Add old call to the erase list
                }
            }

            funcsToErase.push_back(
                Func); // Add dead arg function to the erase list
        }
